resource "aws_iam_role" "role" {
  name = "cola-factory-data-team-run-ctas-athena-role-${var.environment}"
...
resource "aws_iam_policy" "run_athena_policy" {
  name        = "run-athena-policy-${var.environment}"
...
resource "aws_iam_role_policy_attachment" "lambda_s3_run_athena_attachment" {
  ...
  policy_arn = aws_iam_policy.run_athena_policy.arn
}
...
resource "aws_iam_policy" "athena_full_s3_process_bucket_policy" {
  name        = "cola-data-team-athena-full_s3_process_bucket-policy-${var.environment}"
...
resource "aws_iam_policy" "glue_database_policy" {
  name        = "cola-data-team-athena-glue-database-policy-${var.environment}"
